...
let is_keyword = function
  | "let" | "in" | "true" | "false" | "rec" | "fn" | "if" | "else" | "then"
  | "div" | "end" | "val" | "of" | "orelse" | "andalso" | "fun" | "case" ->
      true
  | _ -> false
...
let procr op pl pr =
  let p =
    fix @@ fun p ->
    pl >>= fun l -> op >>= (fun op -> p <|> pr >>| op l) <|> return l
  in
  p
...
let choice_op ops =
  choice
  @@ List.map ~f:(fun (tok, cons) -> token tok *> (return @@ ebinop cons)) ops
...
let cint =
  ...
  return @@ Base.Int.of_string ((if sign == "\r" then "" else "-") ^ uns)
  >>| cint
...
let pack =
  ...
  let other d =
    ...
    let prim =
      empty_lr
      @@ choice [ ptconst; pnone; ptvar; ptwild; plist; brackets @@ d.pt d ]
    in
    let prim_or_some =
      prim <|> (token "SOME" *> prim >>= fun x -> return @@ PtSome x)
    in
...
let pack =
  let pt = pt in
  let exp d =
    fix @@ fun _self -> empty_lr @@ d.key d <|> d.tuple d <|> d.op d
  in
  let key d =
    fix @@ fun _self ->
    let eif =
    ...
    let elet =
      ...
      in
      keyword "let"
      *> empty_lr (elet <$> (many binding <* keyword "in") <*> d.exp d)
      <* keyword "end"
    in
    let efun =
      empty_lr (efun <$> (keyword "fn" *> many pt <* token "=>") <*> d.exp d)
    in
    let ematch =
      let fst_case =
        ccase <$> (option "" (token "|") *> pt <* token "=>") <*> d.exp d
      in
      let other_cases = ccase <$> (token "|" *> pt <* token "=>") <*> d.exp d in
      let cases =
        (fun fst other -> fst :: other) <$> fst_case <*> many other_cases
      in
      let fncases =
        (fun fst other -> fst :: other) <$> fst_case <*> many1 other_cases
      in
      let pmatch =
        ematch <$> (keyword "case" *> d.exp d <* keyword "of") <*> cases
      in
  ...
  let op d =
    ...
    let mul_op =
      procl (choice_op [ ("*", Mul); ("div", Div) ]) app_or_some @@ d.key d
    in
    let add_op =
      procl
        (choice_op [ ("+", Add); ("-", Sub) ])
    ...
    let cmp_op =
      procl
        (choice_op [ (">=", Geq); (">", Gre); ("<=", Leq); ("<", Less) ])
        cons_op
      @@ d.key d
    in
    let eq_op =
      procl (choice_op [ ("=", Eq); ("<>", Neq) ]) cmp_op @@ d.key d
    in
...
let prog =
  sep_by1 (token ";;") decl <* option "" @@ empty_lr (token ";;") >>| pprog
