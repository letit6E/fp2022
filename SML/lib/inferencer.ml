...
module R : sig
  ...
  let ( >>= ) : 'a 'b. 'a t -> ('a -> 'b t) -> 'b t =
   fun monad f state ->
    ...
    match result with Error e -> (last, Error e) | Ok x -> f x last
...
module Type = struct
  ...
  let rec occurs_in v = function
    ...
    | TList typ | TOption typ | TRef typ -> occurs_in v typ
    | _ -> false
  ...
  let unpack_vars =
    ...
    let rec ehelper acc = function
      ...
      | TTuple typ_list ->
          List.fold_right typ_list
            ~f:(fun t s -> Set.union s (ehelper empty t))
            ~init:acc
      | TList typ | TOption typ | TRef typ -> ehelper acc typ
...
module Subst : sig
  ...
  let rec unify l r =
    ...
    | (TVar (a, _), TVar (b, _) | TEqualityVar (a, _), TEqualityVar (b, _))
      when a = b ->
        return empty
    | TVar (b, _), t
    | t, TVar (b, _)
    ...
    | TTuple typ_list_l, TTuple typ_list_r -> (
        ...
        | List.Or_unequal_lengths.Ok zipped_list ->
            List.fold_right zipped_list
              ~f:(fun (x, y) subst ->
                let* head_sub = unify x y in
                let* subst = subst in
                compose head_sub subst)
              ~init:(return empty))
    | TList typ1, TList typ2 | TOption typ1, TOption typ2 | TRef typ1, TRef typ2
      ->
        unify typ1 typ2
...
module Scheme = struct
  ...
  let occurs_in v = function s, t -> (not (Set.mem s v)) && Type.occurs_in v t
  let unpack_vars = function s, t -> Set.diff (Type.unpack_vars t) s
...
let rec is_restricted = function
  ...
  | TTuple list -> (
      ...
      | [] -> false)
  | TList t | TOption t | TRef t -> is_restricted t
  | TGround _ -> false
...
let rec find_identifiers = function
  | PtTuple expr_list ->
      List.fold_right
        ~f:(fun expression acc ->
          Base.Set.union (find_identifiers expression) acc)
        ~init:(Set.empty (module String))
        expr_list
  | PtCons (head, tail) ->
      Base.Set.union (find_identifiers head) (find_identifiers tail)
...
let get_args left right =
  let rec unpack_app acc = function
    ...
    | _ -> acc
  in
  match unpack_app [ right ] left with _ :: tail -> tail | _ -> []

let rec get_name left =
  match left with
  | EApp (l, _) -> get_name l
...
let infer =
  let rec pthelper env = function
    ...
    | PtCons (elem, list) ->
        ...
        let* final_subst =
          Subst.compose_all [ elem_subst; list_subst; subst' ]
        in
    ...
    | PtTuple list ->
        ...
        return
          ( final_subst,
            tuple_t @@ List.map typ_list ~f:(Subst.apply final_subst) )
  in
  let rec ehelper : TypeEnv.t -> exp -> (Subst.t * typ * TypeEnv.t) R.t =
   fun env -> function
    ...
    | EUnOp (op, expr) ->
        let operand_type = match op with Minus -> int_typ | Not -> bool_typ in
    ...
    | ECons (elem, list) ->
        ...
        let* subst' =
          unify (list_t elem_type)
            (match list with ENil -> list_t elem_type | _ -> list_type)
        in
        let* final_subst =
          Subst.compose_all [ elem_subst; list_subst; subst' ]
        in
    ...
    | EMatch (matched_expression, case_list) ->
        ...
        let* subst' =
          List.fold_right case_list ~init:(return Subst.empty)
            ~f:(fun case subst ->
              ...
              let* computation_subst, computation_type, _ =
                ehelper env'' (snd case)
              in
    ...
    | ELet (bindings_list, expression) ->
        let rec process_list subst env = function
          ...
          | elem :: tail ->
              ...
              let* fresh_var = fresh_var in
              let env' =
                TypeEnv.extend env identifier (Set.empty (module Int), fresh_var)
              in
              ...
              process_list subst''
                (TypeEnv.extend env'' identifier generalized_type)
                tail
        in
    ...
    | EApp (EVar "!", EVar id) ->
        let* subst, ttype, _ = lookup_env id env in
        let* type_variable = fresh_var in
        let* subst' = unify (ref_t type_variable) ttype in
        let result_type = Subst.apply subst' type_variable in
        let* final_subst = Subst.compose subst subst' in
        return (final_subst, result_type, env)
    | EApp (EVar "ref", right) ->
        let* right_subst, right_type, _ = ehelper env right in
        return (right_subst, ref_t right_type, env)
    | EApp (EApp (EVar ":=", left), right) ->
        let* left_subst, left_type, _ = ehelper env left in
        let* right_subst, right_type, _ = ehelper env right in
        let* subst' = unify left_type (ref_t right_type) in
        let* final_subst =
          Subst.compose_all [ subst'; left_subst; right_subst ]
        in
        return (final_subst, unit_typ, env)
    | EApp (left, right) ->
        let* subst_left, typ_left, _ = ehelper env left in
        let* subst_right, typ_right, _ =
          ehelper (TypeEnv.apply subst_left env) right
        in
        let* type_variable = fresh_var in
        let get_typ res =
          match res with Ok (_, typ, _) -> typ | _ -> unit_typ
        in
        ...
        let new_typ =
          List.fold args_typs ~init:typ_right ~f:(fun acc x -> TArr (acc, x))
        in
        let* subst' =
          unify
            (arrow_t typ_right type_variable)
            (Subst.apply subst_right typ_left)
        in
        let* fn_name = get_name left in
        let result_type = Subst.apply subst' type_variable in
        let* final_subst =
          Subst.compose_all [ subst_left; subst_right; subst' ]
        in
        let new_final_typ = restrict (TArr (new_typ, result_type)) in
        let new_env' =
          TypeEnv.extend env fn_name (Set.empty (module Int), new_final_typ)
        in
    ...
    | EFun (PtVar head, body) ->
        let* type_variable = fresh_var in
        let env' =
          TypeEnv.extend env head (Set.empty (module Int), type_variable)
        in
    ...
    | EIf (condition, true_branch, false_branch) ->
        ...
        let* false_branch_subst, false_branch_type, _ =
          ehelper env false_branch
        in
...
let check_types environment (dec : decl) =
  ...
  | DLet (true, PtVar name, exp) ->
      ...
      let generalized_type =
        generalize env (Subst.apply final_subst type_variable)
      in
