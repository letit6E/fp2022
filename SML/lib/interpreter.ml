...
let lookup id env = try EnvrMap.find id env with Not_found -> raise Not_bound
...
module type MONAD_FAIL = sig
  ...
  val run :
    ('a, 'e) t -> ok:('a -> ('b, 'e) t) -> err:('e -> ('b, 'e) t) -> ('b, 'e) t
...
module Interpret (M : MONAD_FAIL) = struct
  ...
  let pp_error e =
    ...
    let helper _ = function
      | Match_fail (PtTuple _, TupleVal _) ->
          printf
            "Elaboration failed: Expected and given record types has different \
             number of entries"
      | Match_fail (_, b) ->
      ...
      | Wrong_un_op (un, x) ->
          printf
            "Elaboration failed: Type clash. Unary %s operator cannot take an \
             argument of type %s"
            (show_un_op un) (to_string x)
      | Wrong_bin_op (bn, ex1, ex2) ->
          printf
            "Elaboration failed: Type clash. Binary %s operator cannot take \
             arguments of type %s and %s"
            (show_bin_op bn) (to_string ex1) (to_string ex2)
      | Not_bool_condition _ ->
          printf
            "Elaboration failed: Type clash. If condition must be of type bool"
      | Not_bound id ->
          printf "Elaboration failed: Unbound value identifier %s" id
      | Not_function exp ->
          printf "Elaboration failed: %s is not a function" (show_exp exp)
      | Let_rec_only_vars _ ->
          printf {|Parsing failed: Using "rec" requires a variable|}
  ...
  let lookup_env id env =
    try return (lookup id env) with Not_bound -> fail (Not_bound id)
  ...
  and eval_expr env = function
    ...
    | EMatch (exp, mathchings) ->
        ...
        let rec do_match = function
          | (pt, exp) :: tl ->
              run (pt_match pt evaled)
                ~ok:(fun binds ->
                  let env =
                    extend_state env (List.map (fun (a, b, _) -> (a, b)) binds)
                  in
                  eval_expr env exp)
    ...
    | EApp (exp1, exp2) -> (
        ...
        | EVar "!" -> (
            let* evaled = eval_expr env exp2 in
            match evaled with RefVal x -> return !x | _ -> fail Ref_error)
        | _ -> (
            ...
            | EFun (pt, exp) ->
                let tmp =
                  FunVal (pt, exp, lazy new_state, Info (lab, opt, basic, keys))
                in
                return tmp
    ...
    | EFun (pt, exp) ->
        let* scan_info =
          efun env empty_env empty_env empty_env [] (EFun (pt, exp))
        in
  ...
  and eval_bind env (is_rec, pt, exp) =
    if not is_rec then
      ...
      let* tmp =
        ...
        | EFun (pt, body) ->
            let* scan_info =
              efun env empty_env empty_env empty_env [] (EFun (pt, body))
            in
            let rec new_env =
  ...
  let eval_dec env = function DLet bind -> eval_bind env bind
    ...
    let* binds, typs, _, _ =
      fold
        ~f:(fun (binds, typs, env, environment) dec ->
          ...
          return
            (new_binds :: binds, tcheck_result :: typs, new_state, environment'))
...
module InterpreterResult = struct
  ...
  let run x ~ok ~err = match x with Ok v -> ok v | Error e -> err e
  let ( let* ) x f = x >>= f
end
...
let eval_pp _ code =
  ...
  | Ok prog ->
      InterpreterResult.run (eval_prog prog)
        ~err:(fun x -> pp_error x)
        ~ok:(fun (x, y) ->
          let error_check =
            List.find_opt
              (fun t -> match t with Error _ -> true | _ -> false)
              y
          in
