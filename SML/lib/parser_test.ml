...
let%test _ =
  test_parse ~label:"Fun and val test"
    ~code:
      {|
...
let%test _ =
  test_parse ~label:"Options and matching test"
    ~code:
      {|
...
let%test _ =
  test_parse ~label:"val rec, case test"
    ~code:
      {|
                  ...
                  ( EVar "case",
                    [
                      ...
                      ( PtWild,
                        EApp
                          ( EVar "x",
                            EArg (EBinOp (Sub, EVar "y", EConst (CInt 1))) ) );
                    ] ) ) );
        DLet
          ( false,
            PtVar "t",
            EFun
              ( PtVar "c",
                EMatch
                  ( EVar "c",
                    [
                      (PtConst (CInt 0), EConst (CInt 1337)); (PtWild, EVar "c");
                    ] ) ) );
        DLet
          ( false,
            PtVar "tmp",
            EFun
              ( PtVar "x",
                ETuple
                  [
                    EIf
                      ( EBinOp (Geq, EVar "x", EConst (CInt 2)),
                        ESome (EVar "x"),
                        ENone );
                    EVar "x";
                  ] ) );
          ...
          ( false,
            ...
            EApp
              ( EVar "ref",
                EArg
                  (ETuple
                     [ ESome (EConst (CInt 1)); ENone; EConst (CString "abc") ])
              ) );
      ]

let%test _ =
  test_parse ~label:"fun test"
    ~code:
      {|
                  ...
                  ( EBinOp (Eq, EVar "n", EConst (CInt 0)),
                    ...
                    EBinOp
                      ( Mul,
                        ...
                        EApp
                          ( EVar "fact",
                            EArg (EBinOp (Sub, EVar "n", EConst (CInt 1))) ) )
                  ) ) );
...
let%test _ =
  test_parse ~label:"list and let .. in .. end test"
    ~code:
      {|
      ...
      [
        ...
        DLet
          ( false,
            ...
            ELet
              ( [
                  (false, PtVar "x", EConst (CInt 3));
                  (false, PtVar "y", EConst (CInt 4));
                ],
                EBinOp (Add, EVar "x", EVar "y") ) );
      ]

let%test _ =
  test_parse ~label:"fib test"
    ~code:
      {|
                  ...
                  ( EBinOp
                    ...
                    EBinOp
                      ( Add,
                        ...
                        EApp
                          ( EVar "fib",
                            EArg (EBinOp (Sub, EVar "n", EConst (CInt 2))) ) )
                  ) ) );
        DLet
          ( false,
            PtVar "tmp",
            EApp
              ( EApp
                  ( EApp (EVar "fib", EArg (EConst (CInt 15))),
                    EArg (EConst (CInt 14)) ),
                EArg (EConst (CInt 3)) ) );
        DLet
          ( false,
...
let%test _ =
  test_parse ~label:"app fun and cons test"
    ~code:
      {|
    ...
    ~expected:
      [
        ...
        DLet
          ( false,
            PtVar "t",
            ELet ([], EBinOp (Add, EConst (CInt 3), EConst (CInt 3))) );
      ]

let%test _ =
  test_parse ~label:"let in test"
    ~code:
      {|
            ...
            ELet
              ( [
                  ...
                  ( true,
                    ...
                    EFun
                      ( PtVar "y",
                        EFun (PtVar "z", EBinOp (Add, EVar "y", EVar "z")) ) );
                ],
                EBinOp
                  ( Add,
                    EVar "t",
                    EApp
                      (EApp (EVar "x", EArg (EVar "t")), EArg (EConst (CInt 4)))
                  ) ) );
        DLet
          ( false,
            PtVar "t",
        ...
        DLet
          ( false,
            ...
            EFun
              ( PtVar "x",
                ETuple
                  [ EBinOp (Geq, EVar "x", EConst (CInt 2)); ESome (EVar "x") ]
              ) );
      ]
